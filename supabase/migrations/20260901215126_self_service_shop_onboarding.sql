-- Replace the beta-era manual approval boundary with confirmed-email,
-- self-service workspace creation. Trial duration and tier remain server-owned.
...
begin
  ...
  if current_email_confirmed_at is null then
    raise exception 'Confirm your email before creating a shop workspace.' using errcode = '42501';
  end if;

  -- Serialize first-workspace creation per account so simultaneous requests
  -- cannot each create a separate trial before either owner row is visible.
  ...
  if exists (
    ...
    raise exception 'This account already owns a FretTrack shop.' using errcode = '23505';
  end if;

  if exists (select 1 from public.shop_members where shop_id = normalized_shop_id)
    or exists (select 1 from public.shop_profiles where shop_id = normalized_shop_id) then
    raise exception 'That shop workspace already exists.' using errcode = '23505';
  end if;

  insert into public.shop_profiles (shop_id, shop_name, email, onboarded_at, created_by)
  values (
    normalized_shop_id,
  ...
  select * into saved_subscription
  ...
  values (normalized_shop_id, current_user_id, 'owner', coalesce(current_email, ''))
...
revoke all on function public.bootstrap_current_user_as_owner(text, text) from public, anon, authenticated;
