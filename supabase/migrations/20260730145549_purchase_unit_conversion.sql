...
alter table public.inventory_receipt_items
  add column if not exists purchase_unit text not null default 'each',
  add column if not exists units_per_purchase_unit integer not null default 1,
  add column if not exists inventory_quantity_received integer;

update public.inventory_receipt_items
set inventory_quantity_received = quantity_received
where inventory_quantity_received is null;

alter table public.inventory_receipt_items
  alter column inventory_quantity_received set not null;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'parts_purchase_unit_valid'
      and conrelid = 'public.parts'::regclass
  ) then
    alter table public.parts
      add constraint parts_purchase_unit_valid
      check (purchase_unit in ('each', 'pack', 'box', 'bag', 'case', 'set', 'roll', 'bottle'));
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'parts_units_per_purchase_unit_valid'
      and conrelid = 'public.parts'::regclass
  ) then
    alter table public.parts
      add constraint parts_units_per_purchase_unit_valid
      check (units_per_purchase_unit between 1 and 999999);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'purchase_order_items_purchase_unit_valid'
      and conrelid = 'public.purchase_order_items'::regclass
  ) then
    alter table public.purchase_order_items
      add constraint purchase_order_items_purchase_unit_valid
      check (purchase_unit in ('each', 'pack', 'box', 'bag', 'case', 'set', 'roll', 'bottle'));
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'purchase_order_items_units_per_purchase_unit_valid'
      and conrelid = 'public.purchase_order_items'::regclass
  ) then
    alter table public.purchase_order_items
      add constraint purchase_order_items_units_per_purchase_unit_valid
      check (units_per_purchase_unit between 1 and 999999);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'inventory_receipt_items_purchase_unit_valid'
      and conrelid = 'public.inventory_receipt_items'::regclass
  ) then
    alter table public.inventory_receipt_items
      add constraint inventory_receipt_items_purchase_unit_valid
      check (purchase_unit in ('each', 'pack', 'box', 'bag', 'case', 'set', 'roll', 'bottle'));
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'inventory_receipt_items_units_per_purchase_unit_valid'
      and conrelid = 'public.inventory_receipt_items'::regclass
  ) then
    alter table public.inventory_receipt_items
      add constraint inventory_receipt_items_units_per_purchase_unit_valid
      check (units_per_purchase_unit between 1 and 999999);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'inventory_receipt_items_inventory_quantity_positive'
      and conrelid = 'public.inventory_receipt_items'::regclass
  ) then
    alter table public.inventory_receipt_items
      add constraint inventory_receipt_items_inventory_quantity_positive
      check (inventory_quantity_received between 1 and 999999999);
  end if;
end;
$$;
...
declare
  ...
  safe_purchase_quantity integer;
  safe_inventory_quantity integer;
  safe_purchase_unit_cost numeric(10, 2);
  safe_inventory_unit_cost numeric(10, 2);
begin
  ...
  safe_purchase_quantity := coalesce(p_quantity, 0);
  if safe_purchase_quantity < 1 or safe_purchase_quantity > 999999 then
    raise exception 'Purchase quantity must be between 1 and 999999.';
  end if;
  ...
  if target_part.units_per_purchase_unit < 1 or target_part.units_per_purchase_unit > 999999 then
    raise exception 'Units per purchase unit must be between 1 and 999999.';
  end if;

  if (safe_purchase_quantity::bigint * target_part.units_per_purchase_unit::bigint) > 999999999 then
    raise exception 'Converted inventory quantity is outside the supported range.';
  end if;
  safe_inventory_quantity := (safe_purchase_quantity::bigint * target_part.units_per_purchase_unit::bigint)::integer;
  if safe_inventory_quantity < 1 or safe_inventory_quantity > 999999999 then
    raise exception 'Converted inventory quantity is outside the supported range.';
  end if;

  safe_purchase_unit_cost := round(
    coalesce(p_unit_cost, target_part.unit_cost * target_part.units_per_purchase_unit, 0),
    2
  );
  if safe_purchase_unit_cost < 0 or safe_purchase_unit_cost > 999999.99 then
    raise exception 'Purchase unit cost must be between 0 and 999999.99.';
  end if;
  safe_inventory_unit_cost := round(safe_purchase_unit_cost / target_part.units_per_purchase_unit, 2);

  insert into public.inventory_receipts (
    shop_id, vendor_id, received_by, notes
  )
  values (
    target_part.shop_id,
  ...
  values (
    ...
    safe_purchase_quantity,
    target_part.purchase_unit,
    target_part.units_per_purchase_unit,
    safe_inventory_quantity,
    safe_inventory_unit_cost,
    safe_purchase_unit_cost,
    0,
    safe_inventory_unit_cost
  )
  ...
  update public.parts
  set
    quantity_on_hand = quantity_on_hand + safe_inventory_quantity,
    unit_cost = safe_inventory_unit_cost,
    last_cost = safe_inventory_unit_cost,
    average_cost = round(
      ((greatest(coalesce(target_part.quantity_on_hand, 0), 0) * coalesce(target_part.average_cost, target_part.last_cost, target_part.unit_cost, safe_inventory_unit_cost, 0))
        + (safe_inventory_quantity * safe_inventory_unit_cost))
        / nullif(greatest(coalesce(target_part.quantity_on_hand, 0), 0) + safe_inventory_quantity, 0),
      2
    )
...
declare
  ...
  raw_unit_cost text;
  safe_purchase_unit_cost numeric(10, 2);
  safe_landed_inventory_unit_cost numeric(10, 2);
  safe_shipping_allocated numeric(10, 2);
  remaining_purchase_quantity integer;
  received_rows integer := 0;
  received_purchase_units integer := 0;
  received_inventory_units integer := 0;
...
begin
  ...
  loop
    ...
    safe_purchase_quantity := coalesce(nullif(coalesce(item_payload->>'quantityReceived', item_payload->>'quantity_received', item_payload->>'quantity'), '')::integer, 0);
    ...
    if safe_purchase_quantity < 1 or safe_purchase_quantity > 999999 then
      raise exception 'Purchase receipt quantity must be between 1 and 999999.';
    end if;

    if raw_unit_cost is not null and raw_unit_cost !~ '^[0-9]+(\.[0-9]{1,2})?$' then
      raise exception 'Purchase unit cost must be a non-negative amount with up to two decimals.';
    end if;
    ...
    remaining_purchase_quantity := target_item.quantity_ordered - target_item.quantity_received;
    if safe_purchase_quantity > remaining_purchase_quantity then
      raise exception 'Purchase receipt quantity cannot exceed remaining ordered quantity.';
    end if;

    if target_item.units_per_purchase_unit < 1 or target_item.units_per_purchase_unit > 999999 then
      raise exception 'Units per purchase unit must be between 1 and 999999.';
    end if;

    safe_purchase_unit_cost := round(coalesce(raw_unit_cost::numeric, target_item.unit_cost, 0), 2);
    if safe_purchase_unit_cost < 0 or safe_purchase_unit_cost > 999999.99 then
      raise exception 'Purchase unit cost must be between 0 and 999999.99.';
    end if;

    line_subtotal := safe_purchase_quantity * safe_purchase_unit_cost;
  ...
  insert into public.inventory_receipts (
    shop_id, purchase_order_id, vendor_id, received_by, notes
  )
  values (
  ...
  loop
    ...
    safe_purchase_unit_cost := round(coalesce(raw_unit_cost::numeric, target_item.unit_cost, 0), 2);
    if (safe_purchase_quantity::bigint * target_item.units_per_purchase_unit::bigint) > 999999999 then
      raise exception 'Converted inventory quantity is outside the supported range.';
    end if;
    safe_inventory_quantity := (safe_purchase_quantity::bigint * target_item.units_per_purchase_unit::bigint)::integer;
    if safe_inventory_quantity < 1 or safe_inventory_quantity > 999999999 then
      raise exception 'Converted inventory quantity is outside the supported range.';
    end if;
    line_subtotal := safe_purchase_quantity * safe_purchase_unit_cost;
    ...
    safe_landed_inventory_unit_cost := round(
      ((safe_purchase_quantity * safe_purchase_unit_cost) + safe_shipping_allocated)
        / nullif(safe_inventory_quantity, 0),
      2
    );
    ...
    if target_item.part_id is not null then
      ...
      values (
        ...
        safe_description,
        safe_vendor_sku,
        target_item.purchase_unit,
        target_item.units_per_purchase_unit,
        safe_landed_inventory_unit_cost,
        0,
        0,
        0,
        0,
      ...
      set
        ...
        unit_cost = safe_purchase_unit_cost
    ...
    current_average_cost := coalesce(target_part.average_cost, target_part.last_cost, target_part.unit_cost, safe_landed_inventory_unit_cost, 0);
    next_average_cost := round(
      ((current_quantity_for_average * current_average_cost)
        + (safe_inventory_quantity * safe_landed_inventory_unit_cost))
        / nullif(current_quantity_for_average + safe_inventory_quantity, 0),
      2
    );

    update public.parts
    set
      quantity_on_hand = coalesce(quantity_on_hand, 0) + safe_inventory_quantity,
      unit_cost = safe_landed_inventory_unit_cost,
      last_cost = safe_landed_inventory_unit_cost,
    ...
    update public.purchase_order_items
    set
      quantity_received = quantity_received + safe_purchase_quantity,
      unit_cost = safe_purchase_unit_cost,
      part_id = target_part.id
    where id = target_item.id;

    received_rows := received_rows + 1;
    received_purchase_units := received_purchase_units + safe_purchase_quantity;
    received_inventory_units := received_inventory_units + safe_inventory_quantity;
  end loop;
  ...
  return jsonb_build_object(
    ...
    'receivedPurchaseUnits', received_purchase_units,
    'receivedUnits', received_inventory_units,
