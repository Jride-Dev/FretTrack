-- New workspaces receive the approved standard Pro evaluation: 14 days,
-- no payment method, and no Stripe subscription until the owner explicitly
-- completes Checkout. Existing trials and operator-granted extensions remain
-- unchanged.
...
begin
  ...
  if exists (
    select 1 from public.shop_members where shop_id = normalized_shop_id
  ) or exists (
    select 1 from public.shop_profiles where shop_id = normalized_shop_id
  ) then
