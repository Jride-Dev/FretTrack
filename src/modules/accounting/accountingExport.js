...
export function buildAccountingCsv(report, options = {}) {
  ...
  const rows = [
    ...
    ['Tax Calculation Mode', report.taxProfile?.calculationMode || 'disabled'],
    ['Tax Jurisdiction', report.taxProfile?.jurisdiction || ''],
    ['Tax Registration Number', report.taxProfile?.registrationNumber || ''],
    ['Tax Profile Revision', report.taxProfile?.profileRevision || ''],
    ['Tax Default Rate %', report.taxProfile?.defaultRatePercent ?? ''],
    [],
    ['Summary'],
    ...
    [],
    ['Sales History'],
    ['Date', 'Date ISO', 'Job #', 'Customer', 'Status', 'Currency Code', 'Total Due', 'Paid', 'Balance', 'Tax Amount', 'Tax Jurisdiction'],
    ...(report.salesHistory || []).map((row) => [
      formatShopDate(row.date, dateOptions),
      row.date,
      row.jobNumber,
      row.customerName,
      row.status,
