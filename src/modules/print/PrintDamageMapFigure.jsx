...
export default function PrintDamageMapFigure({ damageMap = {}, viewName }) {
  ...
  const [imageState, setImageState] = useState(() => ({
    url: imageUrl,
    status: imageUrl ? 'loading' : 'missing'
  }));
  const imageStatus = imageState.url === imageUrl
    ? imageState.status
    : (imageUrl ? 'loading' : 'missing');
  ...
  return (
    <section className="print-damage-map-section" data-damage-view={viewName}>
      ...
      {imageUrl && imageStatus !== 'error' ? (
        <div className="print-damage-map-visual">
          <div className="print-damage-map-stage">
            <img
              src={imageUrl}
              alt={`${VIEW_LABELS[viewName] || viewName} condition reference`}
              decoding="sync"
      ...
      {imageStatus === 'loaded' && marks.length > 0 && (
