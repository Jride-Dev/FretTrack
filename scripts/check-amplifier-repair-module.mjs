...
assert.match(jobService, /\.eq\('updated_at', expectedUpdatedAt\)/, 'Amplifier persistence must compare the loaded version atomically with the remote row.');
assert.match(jobService, /FRETTRACK_JOB_SAVE_CONFLICT/, 'Stale amplifier saves must return a recognizable conflict instead of reporting success.');
assert.match(jobService, /if \(expectedUpdatedAt\) \{\s*saveLocalJobs/, 'A version-guarded save must not overwrite the local record until the remote update succeeds.');
