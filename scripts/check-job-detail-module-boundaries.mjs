...
assert.match(detail, /buildJobPrintSections\(\{[\s\S]*?lengthUnit: measurementOptions\.lengthUnit[\s\S]*?onCloseDetail: closeDetail[\s\S]*?onEmailWorkOrder: (?:draftJob\.documentType === 'estimate' \? openEstimateEmail : )?openWorkOrderEmail[\s\S]*?onFinishJob: finishJob[\s\S]*?onPrintCustomerReport: printCustomerReport[\s\S]*?onPrintJobSheet: printJobSheet[\s\S]*?workOrderImages/, 'Print composition must retain shop measurement units, document images, and established action handlers.');
...
assert.match(detail, /patchJob\(buildShopTaxRatePatch\(draftJob, shopSettings\)\)/, 'Applying shop tax must use the complete versioned tax profile.');
...
assert.match(formatting, /function buildTaxFieldPatch\(currentJob, fieldName, fieldValue, inputType = 'text', checked = false\)[\s\S]*?fieldName === 'salesTaxRate' \? \{ rateSource: 'job', calculationMode: 'manual' \}/, 'Editing a tax rate must mark it as a manual job-level override.');
assert.match(formatting, /function buildShopTaxRatePatch\(currentJob, shopSettings = \{\}\)[\s\S]*?profileRevision:[\s\S]*?rateSource: 'shop'/, 'Applying shop tax must copy the complete profile and source metadata.');
...
assert.match(formatting, /function buildAddServicePatch\(currentJob, services, service, serviceId\)[\s\S]*?jobId: currentJob\.id[\s\S]*?quantity: normalizeServiceQuantity\(service\.quantity\)/, 'Service add helper must preserve job identity and normalize quantity behavior.');
assert.match(formatting, /function buildUpdateServicePatch\(services, serviceId, fieldName, value\)[\s\S]*?services\.map\(\(row\) => \(row\.id === serviceId \? \{ \.\.\.row, \[fieldName\]: nextValue \} : row\)\)/, 'Service update helper must update only the selected service row.');
