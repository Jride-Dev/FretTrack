...
assertIncludes(operatorDashboard, "onClick={() => onStartPremiumTrial(shop, days, 'pro')}", 'Operator UI should start Pro trials only.');
...
assertIncludes(legacyBetaUpdate, "if resolved_tier <> 'pro' then", 'Legacy beta subscription RPC must not bypass Pro-only premium trial rules.');
...
const setTrial = functionBody(migration, 'set_shop_premium_trial');
assertIncludes(setTrial, 'if trial_days not in (7, 14, 30) then', 'Start trial must restrict days to 7/14/30.');
assertIncludes(setTrial, "if normalized_tier <> 'pro' then", 'Start trial must restrict tier to Pro.');
...
const extendTrial = functionBody(migration, 'extend_shop_premium_trial');
assertIncludes(extendTrial, 'if extend_days not in (7, 14, 30) then', 'Extend trial must restrict days to 7/14/30.');
assertIncludes(extendTrial, "if resolved_tier <> 'pro' then", 'Extend trial must restrict current trial tier to Pro.');
...
const endTrial = functionBody(migration, 'end_shop_premium_trial');
assertIncludes(endTrial, "'free'", 'End trial must fall back to Free tier.');
assertIncludes(endTrial, "'active'", 'End trial must keep lifecycle active.');
assertIncludes(endTrial, 'trial_ends_at = null', 'End trial must clear trial end.');
assertIncludes(endTrial, 'grace_ends_at = null', 'End trial must clear grace end.');

const snapshot = functionBody(migration, 'get_shop_entitlement_snapshot');
...
assertIncludes(snapshot, "effective_tier := 'free';", 'Expired premium trial should fall back to Free effective tier.');
assertIncludes(snapshot, "entitlement_plan_id := 'free';", 'Expired premium trial should use Free entitlements.');
assertIncludes(snapshot, 'if trial_expired then', 'Expired premium trial should have explicit entitlement handling.');
assertIncludes(snapshot, 'effective_entitlements := entitlement_values;', 'Expired premium trial should ignore overrides and use Free entitlements only.');
assertIncludes(snapshot, "can_write := effective_status not in ('read_only', 'canceled', 'cancelled');", 'Expired premium trial must remain writable unless explicitly read-only/canceled.');
