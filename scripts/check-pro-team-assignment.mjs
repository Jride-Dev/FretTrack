...
assert.ok(jobMutations.includes('toDbJobFromModule(newJob, { includeAssignment: true })'), 'New jobs may persist their optional assignment.');
assert.match(jobMutations, /from\('jobs'\)[\s\S]*\.update\(toDbJobFromModule\(job\)\)/, 'Ordinary job saves must omit assignment fields and avoid stale assignment overwrite.');
