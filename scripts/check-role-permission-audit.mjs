...
assertIncludes(scheduleEventDetails, 'canWrite && <button type="button" onClick={onEdit}', 'Read-only users must not receive schedule edit controls.');
assertIncludes(scheduleEventDetails, 'const canComplete = canWrite', 'Schedule completion controls must remain permission gated.');
assertIncludes(scheduleEventDetails, 'const canReopen = canWrite', 'Schedule reopen controls must remain permission gated.');
