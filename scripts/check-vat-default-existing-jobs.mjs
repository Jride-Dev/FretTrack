...
const jobDetailDerivedState = read('src/modules/jobs/useJobDetailDerivedState.js');
const jobDetailCommunicationActions = read('src/modules/jobs/jobDetailCommunicationActions.js');
...
assert.match(jobDetailDerivedState, /resolveJobTaxSettings\(draftJob,\s*shopSettings\)/, 'Job Detail must resolve inherited VAT from Shop Settings.');
...
assert.match(jobDetailFormatting, /fieldName === 'salesTaxRate'[\s\S]*rateSource: 'job'/, 'Editing a job VAT rate must create an explicit override.');
assert.match(jobDetailCommunicationActions, /withResolvedJobTaxSettings\(jobToUse,\s*scopedShopSettings\)/, 'Generated documents must resolve current VAT before calculation.');
