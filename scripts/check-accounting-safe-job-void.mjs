...
assert.match(jobMutations, /supabase\.rpc\('set_job_accounting_void'/, 'The client must use the guarded RPC.');
assert.match(jobNormalization, /accountingVoidedAt: job\.accounting_voided_at/, 'Remote jobs must hydrate exclusion state.');
