...
- New Checkout is enabled only for the `sell-us-your-guitar` pilot shop with `STRIPE_BILLING_ENABLED=true`, `STRIPE_BILLING_PILOT_SHOP_IDS=sell-us-your-guitar`, and `STRIPE_REQUIRE_TERMS_ACCEPTANCE=true`. These hosted settings were applied on 2026-08-27 without replacing the existing live `STRIPE_API_KEY`; every other shop remains blocked. The first pilot Checkout must open Stripe's hosted page and display required Terms acceptance before payment is completed. Automatic tax remains disabled until the business's registration and tax obligations are confirmed.
