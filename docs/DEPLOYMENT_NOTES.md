...
- Migration `20260904233235_automatic_inbound_email_routes.sql` is applied in production. It removed the incorrectly assigned shared `repairs@rexaaechae.resend.app` route, rotated any pre-release manual route, backfilled all six existing shops, and installs a shop-profile trigger for future signups. Production verification found six shops, six active routes, six distinct shop IDs, six distinct opaque addresses, and no invalid route format. `send-email` version 47 requires JWT and fails closed if its sending shop lacks a route; `receive-email` version 3 intentionally disables gateway JWT so Resend can reach it, then verifies the raw Svix signature and five-minute timestamp window before creating a service-role client. `RESEND_WEBHOOK_SECRET` and `RESEND_API_KEY` remain server-only. An unsigned production request returned `400`; a signed live-delivery and provider replay remain the final external smoke tests.
