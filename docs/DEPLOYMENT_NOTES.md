...
- The production app is Cloudflare Pages build `096d1b46.frettrack.pages.dev`. The branded app URL and immutable Pages build returned `200 OK`, both referenced `assets/index-7AjrArKF.js`, and that production asset returned `200 OK`.
...
- Production migration history matches the repository through `20260831220418_job_estimate_approval_lifecycle.sql`; the linked migration check reports that the remote database is up to date.
