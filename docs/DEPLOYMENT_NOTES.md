...
- The production app is Cloudflare Pages build `9ee433a4.frettrack.pages.dev`. The branded signup URL now opens account creation when `?signup=1` is present, and failed post-creation workspace profile reloads have a dedicated retry path that does not create a second workspace.
