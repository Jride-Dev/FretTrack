...
- The Resend inbound email database boundary is present in production, but the `receive-email` Edge Function has not been deployed, no production route has been provisioned, and no production webhook secret has been changed. Before enabling it, deploy `receive-email` with JWT verification disabled at the Edge Function gateway, set `RESEND_WEBHOOK_SECRET` and `RESEND_API_KEY` as server-only secrets, and provision each receiving address through the service-managed route table. Keep the adapter disabled until the signed-delivery smoke test and replay test pass.
