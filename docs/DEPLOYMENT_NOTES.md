...
- Migration `20260904233235_automatic_inbound_email_routes.sql` is applied in production. It removed the incorrectly assigned shared `repairs@rexaaechae.resend.app` route, rotated any pre-release manual route, backfilled all six existing shops, and installs a shop-profile trigger for future signups. Production verification found six shops, six active routes, six distinct shop IDs, six distinct opaque addresses, and no invalid route format. `send-email` requires JWT and fails closed if its sending shop lacks a route; `receive-email` version 6 intentionally disables gateway JWT so Resend can reach it, then verifies the raw Svix signature and five-minute timestamp window before creating a service-role client. The first signed live delivery reached FretTrack on September 4, 2026, but failed before message insertion because the outbound `RESEND_API_KEY` has Sending access and cannot retrieve received content. A separate Full access `RESEND_RECEIVING_API_KEY` is now stored server-side and used exclusively for received-content retrieval. The first replay proved that retrieval succeeds and exposed a second type mismatch: Resend's text event identity was being written into the outbound-only UUID `request_id`. Version 6 leaves that field empty for inbound correspondence while retaining replay safety in the provider event ledger and provider message identity. The second replay returned HTTP 200, changed the event to `received`, and created exactly one inbound message for `default-shop`; duplicate verification found one event row, one linked message row, and one provider-message identity row. The message matched an existing customer and remains deliberately unassigned to a work order for review in the dedicated Messages workspace. The sending key and webhook signing secret remain unchanged and server-only.
- The dedicated Messages workspace is deployed to Pages build `8b064c54.frettrack.pages.dev`, and the matching public instructions are deployed in landing/docs Worker version `82950d13-086d-4111-9de1-c8bc037a70e8`. Production checks returned `200` for the branded app, Customers guide, and release notes; the branded app served bundle `index-CPHcQ5fs.js`, and both public pages contained the current inbound-reply instructions. The shop-wide Unassigned Inbox no longer renders above every selected customer profile.
