...
- Current branch checked during this review: `main` after Stripe webhook lease-recovery merge `a7b4713`.
...
- Production migration history matches the repository through `20260827194500_stripe_webhook_claim_lease_recovery.sql`; a post-deployment dry run reported the remote database up to date. PR `#217` makes unfinished Stripe event claims retryable, releases failed finalization claims with a processing-token guard, and permits safe reclamation after a five-minute abandoned lease.
