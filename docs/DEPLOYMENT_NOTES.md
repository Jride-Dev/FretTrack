...
- The production app is Cloudflare Pages build `f99f3286.frettrack.pages.dev`. The branded signup URL now opens account creation when `?signup=1` is present, and failed post-creation workspace profile reloads have a dedicated retry path that does not create a second workspace.
...
- Production migration history matches the repository through `20260903062624_service_quantity_whole_numbers.sql`; the linked migration check reports that the remote database is up to date, and the production read-only data-integrity gate passes. The new `job_services` guard enforces quantities from 1 through 9999 as whole numbers for new writes while preserving legacy fractional rows for review.
