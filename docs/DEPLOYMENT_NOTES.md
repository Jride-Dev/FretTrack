...
- The Resend inbound email database boundary and `receive-email` Edge Function version 1 are present in production. The gateway intentionally has JWT verification disabled so Resend can reach it; the function verifies the raw Svix signature and five-minute timestamp window before creating a service-role client. `RESEND_WEBHOOK_SECRET` and `RESEND_API_KEY` are configured as server-only secrets, and `repairs@rexaaechae.resend.app` is the active route for `sell-us-your-guitar`. An unsigned production request returned `400`, and the signed live-delivery/replay smoke test remains required before calling the adapter fully verified.
