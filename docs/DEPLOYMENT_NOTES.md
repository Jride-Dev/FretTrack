...
- The production app is Cloudflare Pages build `f703cdb9.frettrack.pages.dev`. The branded signup URL now opens account creation when `?signup=1` is present, and failed post-creation workspace profile reloads have a dedicated retry path that does not create a second workspace.
...
- Production migration history matches the repository through `20260903071814_public_estimate_approval_links.sql`; the linked migration check reports that the remote database is up to date, and the production read-only data-integrity gate passes. The `job_services` guard enforces quantities from 1 through 9999 as whole numbers for new writes while preserving legacy fractional rows for review.
...
- The Resend inbound email adapter is prepared in PR review only. It has not been deployed, no production route has been provisioned, and no production webhook secret has been changed. Before enabling it, apply migration `20260904082220_resend_inbound_email_adapter.sql`, deploy `receive-email` with JWT verification disabled at the Edge Function gateway, set `RESEND_WEBHOOK_SECRET` and `RESEND_API_KEY` as server-only secrets, and provision each receiving address through the service-managed route table. Keep the adapter disabled until the signed-delivery smoke test and replay test pass.
