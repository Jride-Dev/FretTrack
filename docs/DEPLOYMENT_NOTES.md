...
- The production app is Cloudflare Pages build `89b9fa1d.frettrack.pages.dev`. The branded app URL and immutable Pages build returned `200 OK`, both referenced `assets/index-DnLaKgJu.js`, and the modern/legacy Inventory and Specialist Purchasing assets returned `200 OK` with the package-price correction.
...
- Production migration history matches the repository through `20260831234500_inventory_purchase_unit_cost.sql`; the linked migration check and post-deploy dry run report that the remote database is up to date.
- The public landing/docs Worker is deployed as version `7dcd6384-0810-4d09-ae89-f95a22abf516` and serves stable 0.3.1 access, pricing, annual savings, Terms, Privacy, Support, product guides, community links, release notes, and legacy documentation aliases.
