...
- Production migration history matches the repository through `20260904082220_resend_inbound_email_adapter.sql`. Before applying the six pending commerce, correspondence, and inbound-adapter migrations, snapshot `backups/hosted-supabase-20260904-041017` captured the database, 89 migration records, all 241 Storage objects, the local Docker volume, and 4,972 checksummed files with no failure marker or checksum mismatch. The linked migration check, empty push dry run, data-integrity gate, permission/RPC checks, commerce checks, billing reconciliation checks, correspondence checks, inbound-adapter checks, and tax-profile boundary checks all pass after deployment. Work order `26227-001` retained its manual 20% VAT configuration, and no finalized invoice data required backfill.
...
- The Resend inbound email database boundary and `receive-email` Edge Function version 1 are present in production. The gateway intentionally has JWT verification disabled so Resend can reach it; the function verifies the raw Svix signature and five-minute timestamp window before creating a service-role client. `RESEND_WEBHOOK_SECRET` and `RESEND_API_KEY` are configured as server-only secrets. The incorrectly assigned shared `repairs@rexaaechae.resend.app` route was removed; production currently has no active route. Migration `20260904233235_automatic_inbound_email_routes.sql` and the updated email functions must pass review before production receives automatic, unique per-shop routes. An unsigned production request returned `400`, and the signed live-delivery/replay smoke test remains required after deployment.
