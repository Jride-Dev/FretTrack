...
The older `transaction_events` and `payment_events` backbone is still not a general ledger and does not yet replace work-order payment storage. The payment screen now supports owner/admin-only linked refunds and payment voids: each adjustment identifies its original payment, requires a reason, caps the amount at the remaining balance, and records the remaining balance in the immutable job history. FretTrack records the shop's provider action; it does not call a card processor or claim that a provider refund succeeded. Secure public invoice links remain a separate follow-on slice.

Accounting / Reports now exposes the active shop tax profile alongside a selected-range Sales History. The profile summary is presentation-only: it does not recalculate stored invoice snapshots, determine legal tax treatment, or replace a tax professional. Sales History is derived from the same non-voided job snapshots as totals and exports one row per work order; filters are for operational review and do not mutate billing records. When historical jobs contain explicit tax overrides or mixed profile revisions, the report surfaces that fact so the exported handoff can be reviewed before filing.

The operator dashboard also exposes a read-only billing reconciliation queue. It flags missing subscription/provider identifiers and status or period mismatches for support investigation, while masking provider IDs in the UI. The guarded operator RPC does not mutate subscriptions or contact Stripe; remediation remains in the signed webhook and support workflow.
