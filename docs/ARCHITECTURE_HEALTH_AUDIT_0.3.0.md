...
- Inventory History, Barcode Labels, Vendors, and the Parts search/list now have focused presentation boundaries. `InventoryPage.jsx` remains the controller for part/vendor mutations, image handling, stock adjustments, barcode printing, and purchasing state, reducing its size from approximately 1,619 to 1,337 lines without moving transactional behavior.
