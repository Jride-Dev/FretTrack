...
- The interim `JobDamageReportView.jsx` presentation has been retired. Customer-facing condition output now lives under `modules/print` in `PrintDamageReport.jsx`, `PrintDamageMapFigure.jsx`, and `PrintStyles.css`; marker edits and image persistence remain in Job Detail while the print document owns its image coordinate stage and page layout.
- `JobPrintDocuments.jsx` now composes the isolated `PrintJobSheet.jsx` and `PrintDamageReport.jsx` document renderers. The retired `modules/jobs/JobPrintSheet.js` no longer shares the interactive/global print layout. Print commands, document-email generation, measurement settings, totals, and job persistence remain controlled by Job Detail and their established helpers.
...
- `GuitarJobDetail.jsx` now gives stringed-instrument work a focused, lazy-loaded bench boundary built from the established identity, neck-inspection, string-gauge, Damage Map, photo, and job-update helpers. It deliberately switches into the existing shared Job Detail for parts, services, payments, messages, scheduling, photos, print documents, and history instead of creating a parallel commercial workflow.
...
- Photo display recovery and upload-result merging now live under `modules/photos` (`useJobImageUrl.js` and `photoState.js`). Job Detail no longer replaces the complete working draft or clears its dirty state when a Damage Map upload finishes, while gallery and Damage Map images can regenerate signed display URLs from their durable storage paths after job navigation.
