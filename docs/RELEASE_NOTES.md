...
The first inbound correspondence adapter is implemented as a Resend email ingress. It verifies the raw webhook body with Resend/Svix signatures and a bounded timestamp window, claims provider events in a service-only replay ledger, retrieves full received-email content when needed, and deduplicates by provider message identity. Delivery is accepted only for explicitly provisioned shop routes; unmatched messages are acknowledged without creating customer history, and an inbound message is attached to a customer only when the shop has one exact normalized email match. Inbound records begin unassigned so staff can deliberately route them later. No SMS adapter, browser Realtime subscription, or provider credential is exposed to the client; production enablement remains explicitly controlled per shop route.

Outbound immediate and Scheduled Email sends now use the shop's active inbound route as Resend's Reply-To address. Customers can therefore press Reply and enter the signed inbound flow without learning or typing a separate address, while shops without a provisioned route keep the previous outbound behavior.
