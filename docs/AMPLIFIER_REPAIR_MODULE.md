...
- Create a vendor purchase order directly from the amplifier bench, track vendor package quantity separately from the quantity required by the repair, receive it through Inventory, and explicitly add the received job quantity to Parts & Payments.
...
Migration `20260822033718_specialist_purchasing_bridge.sql` connects the amplifier bench to the established vendor, purchase-order, Inventory receiving, stock-movement, job-parts, and billing records. Purchase creation is shop-scoped, Pro-entitlement checked, and idempotent under concurrent reuse of the same request key. Fulfillment is available only after receiving and returns the existing billing part on retry, so repeated actions cannot consume or bill the same linked purchase twice. A successful fulfillment refreshes both the bench draft and the app-level selected job, making the new Parts & Payments row visible immediately.
