...
- Added 0.2.9-C Customer CSV Import MVP: owners/admins can download a template, upload CSV customer lists, adjust column mappings, preview validation/duplicates, import valid rows with `import_source=csv` and a shared `import_batch_id`, and download skipped/error rows. CSV is supported now; XLSX, vendor import, inventory import, and rollback remain future work.
